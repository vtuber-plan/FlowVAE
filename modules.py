...
class PreConv(nn.Module):
    def __init__(self, in_channels, out_channels, hidden_channels):
        super().__init__()
        self.instance_convs = torch.nn.Sequential(
            torch.nn.Conv1d(in_channels, hidden_channels,
                            kernel_size=1, bias=False),
            torch.nn.LeakyReLU(0.1),
            torch.nn.InstanceNorm1d(hidden_channels, affine=False),

            torch.nn.Conv1d(hidden_channels, hidden_channels,
                            kernel_size=1, bias=False),
            torch.nn.LeakyReLU(0.1),
            torch.nn.InstanceNorm1d(hidden_channels, affine=False),

            torch.nn.Conv1d(hidden_channels, out_channels,
                            kernel_size=1, bias=False),
            torch.nn.LeakyReLU(0.1),
            # torch.nn.InstanceNorm1d(out_channels, affine=False),
        )

    # feat [B, C, T]
    def forward(self, x):
        return self.instance_convs(x)

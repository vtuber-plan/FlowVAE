...
class SynthesizerTrn(nn.Module):
  ...
  def __init__(self, 
    ...
    **kwargs):
    ...
    self.n_speakers = n_spkeaer
    self.gin_channels = gin_channels

    self.use_sdp = use_sdp

    self.enc_p = HubertEncoder(
        inter_channels,
        hidden_channels,
        filter_channels,
        n_heads,
  ...
  def forward(self, x, x_lengths, y, y_lengths, sid=None):
    ...
    # 22/11/16 添加插值处理
    m_p = F.interpolate(m_p, size=(y.shape[2],), mode="nearest")
    logs_p = F.interpolate(logs_p, size=(y.shape[2],), mode="nearest")

...
class AudioSpeakerHubertLoader(torch.utils.data.Dataset):
    """
    ...
    def get_unit_audio_speaker_pair(self, audiopath_sid_unit):
        ...
        audiopath, sid, unitpath = audiopath_sid_unit[0], audiopath_sid_unit[1], audiopath_sid_unit[2]
...
class AudioSpeakerUnitCollate():
    ...
    def __call__(self, batch):
        ...
        unit_padded = torch.FloatTensor(len(batch), batch[0][0].size(0), max_unit_len)
        spec_padded = torch.FloatTensor(len(batch), batch[0][1].size(0), max_spec_len)
        wav_padded = torch.FloatTensor(len(batch), 1, max_wav_len)
        sid = torch.FloatTensor(len(batch), batch[0][3].size(0))
        unit_padded.zero_()
        spec_padded.zero_()
        wav_padded.zero_()
